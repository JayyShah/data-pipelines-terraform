...
resource "aws_s3_bucket" "terraform-job-data-bucket" {
  bucket = "terraform-job-data-bucket"
}
    ...
    Statement = [
      {
        ...
        Resource = aws_s3_bucket.terraform-job-data-bucket.arn
      }
    ]
  })
}
...
resource "aws_lambda_permission" "allow_bucket_execution" {
  ...
  source_arn    = aws_s3_bucket.terraform-job-data-bucket.arn
}

resource "aws_s3_bucket_notification" "bucket_notification" {
  bucket = aws_s3_bucket.terraform-job-data-bucket.id
...
module "glue" {
  source = "./glue"
  terraform-job-data-bucket-access-policy-arn = aws_iam_policy.s3_access_policy.arn
}