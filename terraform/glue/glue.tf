...
variable "terraform-job-data-bucket-access-policy-arn" {
    type = string
}

# Create a new bucket to store the job script

resource "aws_s3_bucket" "terraform-jobs-project" {
  bucket = "terraform-jobs-project"
}

# UPLOAD THE SPARK JOB FILE myjob.py to s3

resource "aws_s3_object" "myjob" {
  bucket = aws_s3_bucket.terraform-jobs-project.id
  key    = "myjob.py"
  source = "myjob.py"
}
...
resource "aws_iam_policy_attachment" "s3_access_attachment_glue" {
  ...
  policy_arn = var.terraform-job-data-bucket-access-policy-arn
...
resource "aws_glue_job" "myjob" {
  ...
  glue_version = "4.0"
  command {
    script_location = "s3://${aws_s3_bucket.terraform-jobs-project.id}/myjob.py"
  }
  default_arguments = {
    "--job-language" = "python"
